CREATE OR REPLACE VIEW `scratch_christian_homberg_ttl120d.partition_row_counts_pipe3_backup` AS (
  WITH
  published_partitions AS (
    SELECT
      *
    FROM
      `world-fishing-827.backup_ttl_60d_pipe_ais_v3_alpha.INFORMATION_SCHEMA.PARTITIONS` ),
  ...
  internal_shards AS (
    SELECT 
      ...
      '' AS storage_tier
    FROM `world-fishing-827.backup_ttl_60d_pipe_ais_v3_alpha.__TABLES__`
