WITH
pipe_production_v20201001 AS (
  SELECT
    *
  FROM
    `world-fishing-827.pipe_production_v20201001.INFORMATION_SCHEMA.PARTITIONS`
  WHERE
...
internal_shards AS (
  SELECT 
    ...
    '' AS storage_tier
  FROM `world-fishing-827.pipe_production_v20201001.__TABLES__`
  WHERE table_id  LIKE '%_20%'
  AND table_id NOT LIKE '%_2012'
    ),
all_partitions AS (
  SELECT * FROM pipe_production_v20201001
  UNION ALL
...
all_partitions_with_published_messages_count AS(
  ...
  *, 
  ...
  SUM(CASE WHEN schema_table_name = "pipe_production_v20201001.messages_segmented" THEN total_rows_safe ELSE 0 END) OVER (PARTITION BY partition_month) total_rows_month_baseline
