...
- Zero-fill actuals for dead-source days. The actuals MERGE used to insert only the rows the per-config source query returned; when a feed stopped publishing (e.g. `marinetraffic` on 2026-01-01, `ais-listener` on 2026-05-04) the source query returned zero rows for the affected timestamps, the SCD2 table accumulated a hole, the forecast model trained only on pre-death values, and the resulting alert never resolved (`t_deltas` coalesces the missing actual to 0 but the forecast stayed at the pre-death level forever — 66M for marinetraffic). `forecast.R` now wraps `select_timestamp_value_sql` in a gap-fill `LEFT JOIN`: a `(missing_timestamps × known_dims)` grid is the LHS, the original source-aggregation subquery is the RHS, and `IFNULL(raw.value, 0)` fills every grid cell the source didn't cover. `known_dims` comes from a new `get_known_dims_recent` helper in `helpers.R` and is restricted to dims with at least one actuals row in the last 180 days so truly-retired dims aren't resurrected. Applies to both the auto-generated and custom-`source_sql` branches (both already produce the same three-column `(timestamp, value, dimension_split_value)` contract). After the next cron run for each affected config the forecast retrains on the now-complete actuals; marinetraffic's median forecast collapses to 0 immediately (its rolling-origin training window is fully populated with zeros), ais-listener decays within ~7 days as 16 zeros accumulate in the 30-period sliding window.
